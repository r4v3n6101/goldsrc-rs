...
fn lump_content<R: Read + Seek, T>(
    ...
    reader.seek(SeekFrom::Start(lump.offset.into()))?;
...
fn model<R: Read>(reader: &mut R) -> io::Result<Model> {
    Ok(Model {
        bounds: bboxf(reader)?,
        origin: vec3f(reader)?,
        nodes: array::try_from_fn(|_| reader.read_i32::<LittleEndian>())?,
        vis_leafs: reader.read_i32::<LittleEndian>()?,
...
fn textures<R: Read + Seek>(mut reader: R, lump: &Lump) -> io::Result<Vec<MipTexture>> {
    ...
    for offset in offsets {
        reader.seek(SeekFrom::Start(begin + u64::from(offset)))?;
